...
<AccordionGroup>
  <Accordion title="Dry-run behavior">
    ...
    - Builder mode (`--ref-*` or `--provider-*` flags): runs full schema validation plus SecretRef resolvability checks for changed refs/providers, so an unknown or invalid target path fails the dry run the same way the real write does.
